...
- keyboard navigation, help, history and both confirmation screens;
...
3. Verify arrow navigation, `/` search, `H` history and `?` help.
4. Open Details, expand/collapse subprocess nodes and return with `Esc`.
5. Open Restart and Force Stop confirmation, verify exact PID/start time, then
   cancel with `N`.
6. Quit with `Q`; verify cursor, echo, canonical input and the original screen
   are restored.
7. Repeat without `--no-mouse`; verify wheel/click mapping. Repeat with
   `--no-mouse`; verify ordinary terminal text selection remains available.
8. Repeat with `NO_COLOR=1` and confirm all states remain textual.
